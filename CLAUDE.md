...
本项目的完整执行规则定义在 **RUNBOOK.md** 中。在修改任何代码之前，必须先理解 RUNBOOK.md 的约束。
...
控制服务器上的部署脚本：
- `scripts/server_pull_run.sh` — 常驻守护进程，每 30 秒轮询，带超时保护
...
6. 控制服务器会在 30 秒内自动拉取并执行，结果写入 `~/codex_pull_logs/latest.log`
